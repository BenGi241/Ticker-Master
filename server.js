...
// Generate full AI analysis for all sections
app.post('/api/analyze/full/:ticker', async (req, res) => {
      try {
            const { ticker } = req.params;
            const { financialData, newsData } = req.body;

            const cacheKey = `analysis:${ticker.toUpperCase()}:full`;
            ...
            console.log(`Generating full AI analysis for ${ticker}...`);
            const data = await geminiAnalysisAPI.analyzeCompany(ticker, financialData, newsData);
            ...
            res.status(500).json({
                  error: error.message,
                  details: 'Failed to generate full AI analysis'
            });
      }
});
