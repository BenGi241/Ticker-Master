// ========================================
// SEC API Utilities (Ported for React)
// ========================================
...
export function processSECData(secJson, periodType) {
      ...
      for (let year = currentYear; year >= startYear; year--) {
            ...
            if (periodType === 'annual') {
                  ...
                  let eps = findFactValue(usGaap, XBRL_TAGS.eps, yearStr);
                  const shares = findFactValue(usGaap, XBRL_TAGS.sharesOutstanding, yearStr);

                  // Compute EPS if missing but we have the components
                  if (eps === 0 && netIncome > 0 && shares > 0) {
                  ...
                  for (let q = 1; q <= 4; q++) {
                        ...
                        let eps = findFactValue(usGaap, XBRL_TAGS.eps, yearStr, q);
                        const shares = findFactValue(usGaap, XBRL_TAGS.sharesOutstanding, yearStr, q);

                        // Compute EPS if missing
                        if (eps === 0 && netIncome > 0 && shares > 0) {
                              eps = netIncome / shares;
                        }
...
// Get CIK from Ticker (Using Backend Proxy)
